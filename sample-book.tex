...
That's probably because neither Bembo, Gill Sans, nor any substitutes were freely available.
Nowadays there are good substitutes are available on CTAN\@.
Hence this version of \TL\ uses \docpkgdef{ETbb} for a Bembo-like roman font, \docpkgdef{gillius} for a \textsf{Gill-like sans serif font}, and \docpkgdef{FiraMono} as a \texttt{monospaced font}.

However this sample book overrides the default \docpkg{FiraMono} it with \texttt{RecursiveMono} font.
It uses the provided \docfilehook{custom-tufte-common.tex}{common} file hook to override fonts, showcasing how to use file hooks.
I have done that because I like the Recursive Mono font better than the Fira Mono font, but this version of Recursive font is not available on CTAN\@.
If you wish to use it, you can use the font files that come with this package in the \texttt{fonts} directory.

This \TL\ class tries to make documents compiled with \iXeLaTeX, \iLuaLaTeX, and \iPdfLaTeX\ look as close to each other as possible.
However, there's no easy way to make the Recursive font work with \hologo{pdfLaTeX}, so that's the main difference between documents.
Additionally, there seem to be small differences between how the engines typeset text, font encoding, and that some use \docpkgdef{fontspec} while other use \docpkgdef{fontenc} packages.
...
To accomplish this using this style, use the \doccmddef{newthought} command:doccmd
\begin{docspec}
