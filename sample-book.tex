...
% `Define options for every class' and `Justification options for margin notes'
% sections in the `tufte-common.def' file
% Alternatively compile this document with the `debug' and search log output for
% `Tufte-LaTeX settings'
% The sample book should also contain current list of options in `Document Class
% Options' section
\documentclass[a4paper]{tufte-book}

% To add text to blank pages from \cleardoublepage uncomment this command
...
% Adds cleverer cross-referencing commands
...
\usepackage{graphicx}
  ...
  % Define base paths for graphics
  \graphicspath{{graphics/}}

% This package improves the quality of tables in LaTeX
% It provides useful commands, and ads some behind the scenes optimizations
\usepackage{booktabs}

% This package provides a way to set units in typographically correct way
...
% The \nicefrac can take optional argument to set the font command
\usepackage{units}

% This package tries to intuit whether to add a space after a command or not
\usepackage{xspace}

% This package extends verbatim with new environments, and more features
\usepackage{fancyvrb}
  % Set default verbatim font, smaller than default
...
% Prints argument within hanging parentheses (i.e., parentheses that take up no
% horizontal space)
% Useful in tabular environments where space is at a premium
...
% Shortcut for printing months name (e.g., January) and then year (e.g., 2008)
% Based on current date, used on the copyright page to mark first printing
\newcommand{\monthyear}{%
  \ifcase\month%
    \or January%
    \or February%
...
% Inserts a blank page with `empty' page style (no folios, heads, or footers)
...
% Highlights text in Maroon color (dark-red)
...
% Halfquad adds a space equal to half a quad space (space in current font size)
\newcommand{\hquad}{\hskip0.5em\relax}

% Prints a TODO item in bold red followed by the text of the todo
...
\newcommand{\tXeLaTeX}{\XeLaTeX\index{XeLaTeX@\protect\XeLaTeX}\xspace}
% Prints formatted LuaLaTeX logo
\providecommand{\LuaLaTeX}{Lua\LaTeX}
% Prints formatted LuaLaTeX logo and adds it to the index
\newcommand{\tLuaLaTeX}{\LuaLaTeX\index{LuaLaTeX@\protect\LuaLaTeX}}
% Prints a backslash from T1 encoding, from fontenc package (I guess?)
\newcommand{\tuftebs}{\symbol{'134}}

% Print command in mono font, in hlred color, with backlash, add it to the index
...
\newenvironment{docspec}
  ...
  {\end{quotation}}
% Print environment, in mono font, in hlred color, add it to the index
% Label the environment with `env:' prefix
\newcommand{\docenvdef}[1]{%
  \hlred{\texttt{#1}}\label{env:#1}%
...
% Print package name, in mono font, in hlred color, add it to the index
% Label the package with `pkg:' prefix
\newcommand{\docpkgdef}[1]{%
  \hlred{\texttt{#1}}\label{pkg:#1}%
  % Index the package
...
% Print document class option, in mono font, in hlred color, add it to the index
% Label the class option with `clsopt:' prefix
\newcommand{\docclsoptdef}[1]{
  \hlred{\texttt{#1}}\label{clsopt:#1}%
...
% Error gets printed in mono font and inside a fullwidth environment
...
First, the pages in front matter are traditionally numbered with lowercase roman numerals (e.g., i, iv, ix,~\ldots). Second, the front matter page numbering sequence is usually separate from the main matter page numbering.
...
\newthought{The tables of contents} in Tufte's books give us our first glimpse of the structure of the main matter. \VDQI is split into two parts, each containing some number of chapters.
...
Tufte's books primarily use two typefaces: Bembo and Gill Sans.
...
That's because neither Bembo, Gill Sans nor any substitutes were freely available.
Nowadays most \LaTeX distributions have them available.
Hence this version of \TL uses \docpkgdef{ETbb} for a Bembo-like serif font, and \docpkgdef{gillius} for a Gill-like sans serif font.
By default it uses \docpkgdef{FiraMono} for monospaced fonts, however provided \docfilehook{custom-tufte-common.tex}{common} overrides it with \texttt{RecursiveMono} font.
This shows how to use filehooks, override fonts, and shows off my favorite monospaced font.

It also attempts to make files compiled with \XeLaTeX, \LuaLaTeX, and pdf\LaTeX look as similar as possible.
However, there seems to be small differences, which are probably caused by the differences between \docpkgdef{fontspec} and \docpkgdef{fontenc} packages, and how they handle font encoding/formatting.
...
\sidenote{Parts and chapters are defined for the \texttt{tufte-book} class only.}
sections, subsections, and paragraphs. 
By default subsubsection and subparagraph headings are not defined in the \TL classes.%
\sidenote{For more information on this topic, see~\cite{Bringhurst2005}, section 4.2.2}
...
This class does not provide pre-defined styles for \Verb|\subsubsection| or \Verb|subparagraph|. 
...
To accomplish this using this style, use the \doccmddef{newthought} command:
\begin{docspec}
...
Any \doccmd{footnote}s will automatically be converted to sidenotes.%
\footnote{This is a sidenote that was entered using the \Verb|footnote| command.}
...
It will change the number of the current sidenote only and will not affect the numbering sequence of subsequent sidenotes.
...
if the dimension is negative, it will pull the sidenote up the page.
...
\sidenote{%
  If you use the \Verb|cite| command within a sidenote, it will render as an in-line parenthetical citation, as demonstrated here \autocite{Tufte2001}.
}

You will need to specify a bibliography resource file in the preamble of your document using \doccmddef{addbibresource}.
The complete list of references may also be printed automatically by using the \doccmddef{printbibliography} command.
See the end of this document for an example, and the Bib\LaTeX{} documentation for more information.
Bibliography can be turned off with the help of \doccmddef{nobibliography} command.

To enter multiple citations at one location,\autocite{Tufte2006,Tufte1990} you can provide a list of keys separated by commas: \Verb|\cite{Tufte2006,Tufte1990}|. 
\begin{docspec}
  \doccmd{cite\{\docarg{bibkey1,bibkey2,\ldots}\}}
\end{docspec}

In the new version of \TL, it's impossible to offset citations the same way sidenotes can be offset.
This is caused by change from \docpkgdef{natbib} and \docpkgdef{bibtex} to \docpkgdef{biblatex} and \docpkgdef{biber} packages.
This switch is motivated by the fact that \docpkg{natbib} is mostly kept on life support, and \docpkg{biblatex} is more powerful and flexible.
However it has its own optional arguments for \doccmd{cite} commands, which are kept unchanged to avoid confusion.
...
\begin{figure}
  ...
  \caption[Hilbert curves of various degrees \(n\)][1em]{%
    Hilbert curves of various degrees \(n\).
    \emph{Notice that this figure only takes up the main textblock width.}
  }\label{fig:textfig}
\end{figure}

As with sidenotes and marginnotes, a caption may require vertical adjustment. 
...
The short caption, if provided, is what appears in the list of figures/tables, otherwise
the ``long'' caption appears there.
Note that although the arguments \docopt{short caption} and \docopt{offset} are both optional, they must be
provided in order. 
...
As an example, the caption to figure~\ref{fig:textfig} above was given in the form
\begin{docspec}
  \doccmd{caption}[Hilbert curves\ldots][1em]\{Hilbert curves\ldots\}
\end{docspec}
Note that caption offset is not available for \docenv{marginfigure} and \docenv{margintable} environments.
...
In cases where a table has many rows, one can use \docpkgdef{colortbl} to make rows stand out visually from each other.
...
\end{docspec}
\LaTeX{} tries to place floats in the best position on the page.
...
\LaTeX{} may run out of ``slots'' to keep track of them and will generate the aforementioned error.

\LaTeX{} initially allocates 18 slots for storing floats. 
...
(This is because allocating more floats may lead to \LaTeX{} running out of memory.)
...
(Each sidenote actually uses \emph{two} slots.)

After placing the floats, \LaTeX{} will mark those slots as unused so they are available for the next page to be composed.


\section{Captions}
You may notice that the captions are sometimes misaligned.
Due to the way \LaTeX's floats works, it's hard to know for sure where it decided to put the float.
Therefore, the \TL document classes provide commands to override the caption position.

\paragraph{Vertical alignment} To override the vertical alignment, use the \doccmd{setfloatalignment} command inside the float environment.
For example:
...
\begin{Verbatim}[commandchars=+\(\)]
  \begin{figure}[btp]
    ...
    \label{fig:sinewave}
    +hlred(\setfloatalignment{b} % forces caption to be bottom-aligned)
  \end{figure}
\end{Verbatim}
\end{fullwidth}
...
\begin{Verbatim}[commandchars=+\(\)]
  \begin{figure}[btp]
    ...
    \caption{This is an example of a sine wave.}%
    \label{fig:sinewave}
    +hlred(\forceversofloat % forces caption to be set to the left of the float)
  \end{figure}
\end{Verbatim}
\end{fullwidth}
...
When using \tXeLaTeX or \tLuaLaTeX, the \TL classes will load the \docpkg{fontspec} package.
...
However the provided \docfilehook{custom-tufte-common.tex}{common} filehook overrides the default monospaced font with \texttt{RecursiveMono} font.
This file shows how you can override the default fonts, and how the filehooks can be used.
...
Under pdf\LaTeX, the \TL classes will try to use the same default fonts, but will fall back to the default Computer Modern fonts if they are not available.
The \docpkg{fontspec} package is not available under pdf\LaTeX, so it uses the \docpkg{fontenc} package to set the font encoding.
...
However if the \docclsopt{nols} was \textbf{not} used, \docpkg{fontspec} will be loaded in \tLuaLaTeX or \tXeLaTeX engines, as they require it to set letterspacing.
...
The \doccmddef{allcaps} command has proper letterspacing for strings of \allcaps{FULL CAPITAL LETTERS}, and the \doccmddef{smallcaps} command has letterspacing for \smallcaps{small capital letters}.
These commands will also automatically convert the case of the text to upper- or lowercase, respectively.

The \doccmddef{textsc} command has also been redefined to include letterspacing.
...
The \doccls{tufte-book} class is based on the \LaTeX\ \doccls{book} document class.
...
In case of \doccls{tufte-handout} these options also have an effect on abstract.
...
You should use this option if you wish to load your own fonts in pdf\LaTeX. 
If you're using \tXeLaTeX or \tLuaLaTeX, the will not be loaded, and you can use \docpkg{fontspec} to set your own.
...
It uses \docpkg{letterspace} or the \docpkg{soul} under pdf\TeX.
In case of \tXeLaTeX and \tLuaLaTeX it uses \docpkg{fontspec}.

The \docclsoptdef{bidi} option loads the \docpkg{bidi} package which is used with \tXeLaTeX to typeset bi-directional text. 
...
The \docclsopt{notoc} option will revert to \LaTeX's \textsc{toc} design.
...
\LaTeX{} allocates 18 slots for storing floats.
The first time \doccmd{morefloats} is called, it allocates an additional 34 slots.
The second time \doccmd{morefloats} is called, it allocates another 26 slots.
...
\begin{multicols}{2}
  \begin{itemize}
    ...
    \item fontenc * \textit{only with pdf\LaTeX, and \texttt{nofonts} is off}
    \item fontspec * \textit{only with \tXeLaTeX or \tLuaLaTeX, and \texttt{nofonts} is off}
    \item geometry
    \item gillius2 * \textit{if available, and \texttt{nofonts} is off}
    \item hardwrap
    \item hyperref * \textit{only if \texttt{nohyper} is off}
    \item iftex * \textit{if not it assumes pdf\LaTeX}
    ...
    \item sectsty
    \item setspace
    \item soul * \textit{only with pdf\LaTeX}
    \item textcase
    \item textcomp * \textit{only with pdf\LaTeX, and \texttt{nofonts} is off}
