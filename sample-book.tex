...
% Print file hook (customization file), in mono font, add it to the index
...
\par\smallcaps{\url{https://salwerowi.cz/Modernized-Tufte-LaTeX}}
...
\begin{table}[h]\index{typefaces!sizes}
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{A list of \hologo{LaTeX} font sizes as defined by the \TL\ document classes.}%
  \label{tab:font-sizes}
\end{table}


\section{Headings}\label{sec:headings}\index{headings}
\newthought{Tufte's books include the following heading levels:} \doccmd{parts}, \doccmd{chapters},%
\sidenote[][-1em]{Parts and chapters are defined for the \texttt{tufte-book} class only.}
\doccmd{sections}, \doccmd{subsections}, and \doccmd{paragraphs}. 
...
The \doccmd{paragraph} headings (as shown here) are introduced by italicized text and separated from the main paragraph by a bit of space.
...
{On the Use of the \texttt{tufte-book} Document Class}\label{ch:use-tufte-book-document-class}
...
To place figures or tables in the margin, use the \docenvdef{marginfigure} or \docenvdef{margintable} environments as follows (see \cref{fig:margin-figure}):
\begin{marginfigure}%
...
\pagebreak
\Cref{fig:full-width-figure} is an example of the \docenv{figure*} environment and \cref{fig:text-figure} is an example of the normal
...
\Cref{tab:normal-table} shows table created with the \docpkg{booktabs} package.
...
\section{Full-width text blocks}\label{sec:full-width-text-blocks}
...
More info in following subsection.
...
\chapter[Customizing Tufte-LaTeX]{Customizing \TL}\label{ch:customizing-tufte-latex}
...
\section{File Hooks}\label{sec:file-hooks}%
\index{file hooks|(}
...
\section{Numbered Section Headings}\label{sec:numbered-section-headings}%
...
\section{Changing the Paper Size}\label{sec:changing-paper-size}
...
\chapter[New Features in Tufte-LaTeX Classes]{New Features in \TL\ Classes}\label{ch:new-features-tufte-latex-classes}
\begin{multicols}{2} [
...
\section{Modified Section Headings}\label{sec:modified-section-headings}
...
\sidenote{Repository can be found at \url{https://github.com/MormonJesus69420/Modernized-Tufte-LaTeX}}.
...
See the \nameref{sec:compiling-your-document} section for more information on compiling the document.
...
\section{Compiling Your Document}\label{sec:compiling-your-document}
The \TL\ classes can be compiled using \iPdfLaTeX, \iXeLaTeX, or \iLuaLaTeX.
I personally recommend using \iLuaLaTeX, as it provides the best support for \TL\ functionality without and quirks that \iXeLaTeX\ has.

The \TL\ classes are designed to work with the \docpkg{biblatex} package for bibliographies.
As such you will need to have \texttt{biber} installed on your system to compile the bibliography files.

\subsection{Compiling with \texttt{makefile}}\label{ssec:compiling-with-makefile}
As mentioned before you can use the provided \texttt{makefile} to compile the sample documents with either \texttt{make compile-lua}, \texttt{make compile-xe}, or \texttt{make compile-pdf}.
It is up to you which engine you prefer to use, but from my experience \iLuaLaTeX\ provides the best results and is the most reliable of them all.
In case of single files use the \texttt{make compile-only FILE="<file>.tex"} target.
It is set up to use \iLuaLaTeX\ by default, but you can change it to use \iXeLaTeX\ or \iPdfLaTeX\ by changing the \texttt{-lualatex} option to \texttt{-xelatex} or \texttt{-pdflatex}.

In general I would recommend changing the \texttt{-outdir=samples} in the \texttt{LMKFLAGS} variable to a directory of your choice, for example: \texttt{-outdir=out}.

If you wish to clean up the auxiliary files, use the \texttt{make clean} target.
And the remove both auxiliary files and compiled PDFs use the \texttt{make clean-all} target.

\newthought{In most usecases you can use this} simplified \texttt{makefile}, with only the necessary targets for most use-cases.
You may use it to compile files using different \hologo{LaTeX} classes too.
As before you can change \iLuaLaTeX\ for other engines.
...
\subsection{Compiling manually}\label{ssec:compiling-manually}
You may also compile the documents with \texttt{latexmk} directly using the following command:
Just substitute ``\texttt{<<engine>>}'' with your preferred engine, and ``\texttt{<<source.tex>>}'' with the name of your source file.
\begin{fullwidth}
\begin{Verbatim}
...
\chapter{Compatibility Issues}\label{ch:compatibility-issues}
...
\section{Converting from \doccls{article} to \doccls{tufte-handout}}\label{sec:converting-article-tufte-handout}
...
\section{Converting from \doccls{book} to \doccls{tufte-book}}\label{sec:converting-book-tufte-book}
...
\chapter{Troubleshooting and Support}\label{ch:troubleshooting-support}
\section{\TL\ Website}\label{sec:tufte-latex-website}
...
I have also set up a custom website for the project which is hosted under \url{https://salwerowi.cz/Modernized-Tufte-LaTeX}.%
\marginnote{If for some reason this address does not work it should also be available under \url{https://mormonjesus69420.github.io/Modernized-Tufte-LaTeX}.}


\section{\TL\ Mailing Lists}\label{sec:tufte-latex-mailing-lists}
There is only one surviving mailing list for the \TL\ project:

\paragraph{Discussion list}\label{par:discussion-list}
...
\paragraph{Commits list}\label{par:commits-list}
...
\section{Errors, Warnings, and Informational Messages}\label{sec:errors-warnings-informational-messages}
...
\section{Package Dependencies}\label{sec:package-dependencies}
