...
Nowadays most \hologo{LaTeX} distributions have them available.
...
It also attempts to make files compiled with \iXeLaTeX, \iLuaLaTeX, and \iPdfLaTeX\ to look as similar as possible.
...
(This is because allocating more floats may lead to \hologo{LaTeX} running out of memory.)
...
\begin{Verbatim}[commandchars=+\(\)]
  \begin{figure}[btp]
    ...
    \caption{This is an example of a sine wave.}%
    \label{fig:sinewave}
    +hlorange(\forceversofloat % forces caption to be set to the left of the float)
  \end{figure}
\end{Verbatim}
\end{fullwidth}
...
Under \iPdfLaTeX, the \TL\ classes will try to use the same default fonts, but will fall back to the default Computer Modern fonts if they are not available.
...
However if the \docclsopt{nols} was \textbf{not} used, \docpkg{fontspec} will be loaded in \hologo{LuaLaTeX} or \hologo{XeLaTeX} engines, as they require it to set letterspacing.
